...
**Status:** Proposed design, 2026-09-20. **Implemented against `FakeChain`:** S0–S2, S5 (three-OS installer CI green 2026-09-20: quote → `/v0/fake/pay` → Credit collects → paid upload → recovery-key re-collect), S6 U5 UI (Top up quotes XMR, Pending list, recovery-key credit-seed copy). **S3 adapter (CI, no monerod):** `WalletRpcChain` + SQLite backup/restore. **S8:** epoch rotate / burn + `POST /v0/exchange` + node `pull_keys`. **S9:** `rsa-bssa-v1` (RFC 9474 A.3 vector + public verify + `pk_tok` spend) as a parallel scheme; lab default stays `ristretto-v0`. **Not started:** S3 live stagenet smoke, S7 live gate 0c, dated gate 0e PASS. Covers gate 0c ([`08-lab.md`](08-lab.md)), U5 ([`09-ui-track.md`](09-ui-track.md)), and three roadmap flags: *issuer trust / key rotation / compromise*, *multi-issuer / settlement*, *pricing & denomination UX* ([`03-roadmap.md`](03-roadmap.md)). Companion ADR: [ADR-0002](adr/0002-payment-attribution-and-token-scheme.md). Owner decisions in §14 are still open.
...
| S3 | ✅ `WalletRpcChain` (`create_address` / `get_transfers` in+pool / `get_height`); issuer `--chain wallet-rpc`; SQLite backup/restore | `payment/chain_walletrpc.py`, `payment/backup.py`, `deploy/zkap-lab/scripts/backup-issuer.sh` | CI: mock JSON-RPC + restore drill. Live stagenet smoke is S7 |
...
| S6 | ✅ U5 UI: Top up quote → pay → poll; Pending list; recovery-key credit-seed copy | `leasegrid_sync/app.py`, `credit.py` | `tests/test_sync_ui.py` (Continue/pay/issued/underpaid/pending/export); `tests/test_sync_credit.py` (gated FakeChain top-up increases balance; unpaid allocate still fails). Faucet is not the Credit button |
...
| S9 | ✅ `rsa-bssa-v1` (RFC 9474 RSABSSA-SHA384-PSS-Deterministic) + `pk_tok` spend binding; node verifies with public key only. Lab default stays `ristretto-v0`. | `rsa_bssa.py`, `issuer.py`, `gate.py` | `tests/test_zkap_rsa_bssa.py` (A.3 vector, public verify, cannot re-bind/mint, settlement `t` only, quote/redeem). Dated gate 0e PASS is still live/manual |
