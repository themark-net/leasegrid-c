"""S9: rsa-bssa-v1 (RFC 9474 RSABSSA-SHA384-PSS-Deterministic) + pk_tok binding.

Lab default stays ristretto-v0. This scheme is what a second operator needs:
the node verifies with the public key only and cannot re-bind a received token.
"""

from __future__ import annotations

import binascii

import pytest

from leasegrid_zkap.crypto import generate_signing_key
from leasegrid_zkap.errors import SpendError
from leasegrid_zkap.gate import LeaseGate
from leasegrid_zkap.r_bind import encode_r
from leasegrid_zkap.spentset import SpentSet


def _unhex(s: str) -> bytes:
    return binascii.unhexlify("".join(s.split()))


# RFC 9474 Appendix A.3 — RSABSSA-SHA384-PSS-Deterministic
_RFC_P = """

7e32ebca06308a12ecc290c7cd1156dcccfb2311
"""
_RFC_Q = """

9803879d2fc4a28e69291d73dbd799f8bc238385
"""
_RFC_N = """

48bdead5
"""
_RFC_E = "010001"
_RFC_D = """

b877a051
"""
_RFC_MSG = """
8f3dc6fb8c4a02f4d6352edf0907822c1210a9b32f9bdda4c45a698c80023a
a6b59f8cfec5fdbb36331372ebefedae7d
"""
_RFC_SALT = """
051722b35f458781397c3a671a7d3bd3096503940e4c4f1aaa269d60300ce
449555cd7340100df9d46944c5356825abf
"""
_RFC_BLINDED = """
10c166c6a711e81c46f45b18e5873cc4f494f003180dd7f115585d

533d8e0594ace7c442
"""
_RFC_BLIND_SIG = """
364f6a40dbfbc3bbb257943337eeff791a0f290898a6791283bba581

c0dc851e1509de1a
"""
_RFC_SIG = """

c706195d52
"""
_RFC_INV = """

5fdec0991f
"""


def test_rfc9474_pss_deterministic_vector():
    from leasegrid_zkap.rsa_bssa import (
        blind,
        blind_sign,
        finalize,
        private_key_from_pqned,
        verify,
    )

    sk = private_key_from_pqned(
        p=int.from_bytes(_unhex(_RFC_P), "big"),
        q=int.from_bytes(_unhex(_RFC_Q), "big"),
        n=int.from_bytes(_unhex(_RFC_N), "big"),
        e=int.from_bytes(_unhex(_RFC_E), "big"),
        d=int.from_bytes(_unhex(_RFC_D), "big"),
    )
    pk = sk.public_key()
    msg = _unhex(_RFC_MSG)
    salt = _unhex(_RFC_SALT)
    inv = int.from_bytes(_unhex(_RFC_INV), "big")
    blinded, got_inv = blind(pk, msg, salt=salt, inv=inv)
    assert blinded == _unhex(_RFC_BLINDED)
    assert got_inv == inv
    blind_sig = blind_sign(sk, blinded)
    assert blind_sig == _unhex(_RFC_BLIND_SIG)
    sig = finalize(pk, msg, blind_sig, inv)
    assert sig == _unhex(_RFC_SIG)
    verify(pk, msg, sig)


def test_roundtrip_and_public_verify():
    from leasegrid_zkap.rsa_bssa import generate_issuer_rsa, issue, token_message, verify

    sk, pk = generate_issuer_rsa()
    t = b"\x11" * 32
    from leasegrid_zkap.rsa_bssa import derive_tok_keypair

    tok_sk, tok_pk = derive_tok_keypair(b"\x22" * 32, "aabbccddeeff0011", 0)
    msg = token_message(t, tok_pk)
    sig = issue(sk, pk, msg)
    verify(pk, msg, sig)


def test_node_verifies_with_public_key_only_and_cannot_rebind(tmp_path):
    from leasegrid_zkap.rsa_bssa import (
        derive_tok_keypair,
        generate_issuer_rsa,
        issue,
        sign_r,
        token_id,
        token_message,
        verify_spend,
    )

    iss_sk, iss_pk = generate_issuer_rsa()
    seed = b"\x33" * 32
    t = token_id(seed, "aabbccddeeff0011", 0)
    tok_sk, tok_pk = derive_tok_keypair(seed, "aabbccddeeff0011", 0)
    sigma = issue(iss_sk, iss_pk, token_message(t, tok_pk))
    r = encode_r(
        nodeid="node-a",
        storage_index=b"\x44" * 16,
        lease_seconds=100,
        share_bytes=64,
        token_epoch=1,
        issuer_pubkey_id="rsa-lab",
    )
    sig_r = sign_r(tok_sk, r)
    verify_spend(iss_pk, t, tok_pk, sigma, r, sig_r)

    # A node that saw the spend has (t, pk_tok, sigma) but not sk_tok.
    r2 = encode_r(
        nodeid="node-b",
        storage_index=b"\x55" * 16,
        lease_seconds=100,
        share_bytes=64,
        token_epoch=1,
        issuer_pubkey_id="rsa-lab",
    )
    with pytest.raises(Exception):
        verify_spend(iss_pk, t, tok_pk, sigma, r2, sig_r)

    # Fabricated sigma (node cannot mint).
    with pytest.raises(Exception):
        verify_spend(iss_pk, t, tok_pk, b"\x00" * len(sigma), r, sig_r)


def test_gate_rsa_spend_and_settlement_is_t_only(tmp_path):
    from leasegrid_zkap.rsa_bssa import (
        derive_tok_keypair,
        generate_issuer_rsa,
        issue,
        rsa_pubkey_id,
        sign_r,
        token_id,
        token_message,
    )

    iss_sk, iss_pk = generate_issuer_rsa()
    pkid = rsa_pubkey_id(iss_pk)
    ristretto_key = generate_signing_key()
    gate = LeaseGate(ristretto_key, nodeid="node-a", spent=SpentSet(str(tmp_path / "s.json")))
    gate.accept_rsa_epoch(1, iss_pk, pkid)
    seed = b"\x66" * 32
    t = token_id(seed, "0011223344556677", 0)
    tok_sk, tok_pk = derive_tok_keypair(seed, "0011223344556677", 0)
    sigma = issue(iss_sk, iss_pk, token_message(t, tok_pk))
    r = encode_r(
        nodeid="node-a",
        storage_index=b"\x77" * 16,
        lease_seconds=100,
        share_bytes=64,
        token_epoch=1,
        issuer_pubkey_id=pkid,
    )
    out = gate.spend_rsa(t, tok_pk, sigma, r, sign_r(tok_sk, r))
    assert out["ok"] is True
    from base64 import b64encode

    assert gate.settlement_preimages() == [b64encode(t).decode("ascii")]
    assert all("|" not in p for p in gate.settlement_preimages())


def test_issuer_quote_redeem_rsa_bssa():
    from leasegrid_zkap.client import ClientError, http_json
    from leasegrid_zkap.issuer import start_issuer
    from leasegrid_zkap.payment import FakeChain, PricePolicy, VoucherStore
    from leasegrid_zkap.rsa_bssa import (
        blind,
        derive_tok_keypair,
        finalize,
        generate_issuer_rsa,
        load_rsa_public_spki,
        token_id,
        token_message,
        verify,
    )

    rsa_sk, rsa_pk = generate_issuer_rsa()
    key = generate_signing_key()
    state, httpd = start_issuer(
        key,
        "127.0.0.1:0",
        chain=FakeChain(),
        policy=PricePolicy(price_piconero=6 * 10**9),
        store=VoucherStore(),
        faucet=False,
        rsa_key=rsa_sk,
    )
    try:
        url = state.listen
        q = http_json(url + "/v0/quote", "POST", {"tokens": 1, "scheme": "rsa-bssa-v1"})
        assert q["scheme"] == "rsa-bssa-v1"
        http_json(url + "/v0/fake/pay", "POST", {"vid": q["vid"], "amount_piconero": 6 * 10**9, "mine": 2})
        seed = b"\x88" * 32
        t = token_id(seed, q["vid"], 0)
        _tok_sk, tok_pk = derive_tok_keypair(seed, q["vid"], 0)
        msg = token_message(t, tok_pk)
        blinded, inv = blind(rsa_pk, msg)
        from base64 import b64decode, b64encode

        issued = http_json(
            url + "/v0/redeem",
            "POST",
            {"vid": q["vid"], "blinded-tokens": [b64encode(blinded).decode("ascii")]},
        )
        assert issued["token-epoch"] == q["epoch"]
        assert issued.get("scheme") == "rsa-bssa-v1"
        assert not issued.get("proof")
        pk = load_rsa_public_spki(issued["public-key"])
        sig = finalize(pk, msg, b64decode(issued["signed-tokens"][0]), inv)
        verify(pk, msg, sig)
        # ristretto default still refused for rsa-only request? default quote is ristretto
        q0 = http_json(url + "/v0/quote", "POST", {"tokens": 1})
        assert q0["scheme"] == "ristretto-v0"
    finally:
        httpd.shutdown()
